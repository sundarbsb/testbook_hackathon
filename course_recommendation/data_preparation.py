...
def prepare_data(df):
    ...
    new_col = []
    for c in df.columns:
        new_col.append(c.lower().replace(' ','_'))
    df.columns=new_col
...
def main():
    ...
    df_overview = pd.read_csv("data\\coursera-courses-overview.csv")
    df_individual = pd.read_csv("data\\coursera-individual-courses.csv")
